...
<div class="row">
        ...
        <div class="list-grid-item rounded">
                ...
                <a :href="'https://crossedlink.com/jobs/'+job.id">
                    ...
                    <div class="grid-list-desc mt-3">
                        ...
                        <p class="text-muted f-14 mb-1 address">{{ job.address }}</p>
                            <p class="text-muted mb-1" v-if="job.salary_range !== null"> {{job.salary_range }}/Month</p>
                ...
                <div class="apply-button p-3 border-top" v-if="role == 0" >
                    <div class="job-detail rounded">
                        <form action="#" @submit.prevent="apply('#apply'+job.id)" :id="'apply'+job.id" class="unapply" v-if="job.applied == 1 && complete.length <= 0">
                            ...
                            <button  @click="apply('#apply'+job.id)" class="btn btn-primary btn-sm" >Unapply</button>
                        </form>
                        <div v-else>
                            <form action="#" @submit.prevent="apply('#apply'+job.id)" :id="'apply'+job.id" class="apply" v-if="job.canApply == 1 && job.applied == 0 && complete.length <= 0 ">
                                <input type="hidden" name="job_id" :value="job.id">
                                <!--<input type="submit" class="btn btn-primary btn-sm" value="Apply Now" >-->
                                <button  @click="apply('#apply'+job.id)" class="btn btn-primary btn-sm" >Apply Now</button>
                            </form>
                            
                            <button type="button" class="btn btn-primary btn-sm" data-toggle="modal"  @click="toggle('#cant_apply' + job.id)"  data-target="#cant_apply" v-else>
                                Apply Now
                            </button>
                        </div>
                    </div>
          ...
          <div class="modal" :id="'cant_apply' + job.id" v-if="role == 0">
                        ...
                        <div class="modal-content">
                            ...
                            <h5 class="modal-title" id="exampleModalLabel">Complete Your Profile First!</h5>
                            ...
                            <button type="button" class="btn btn-secondary " @click="close('#cant_apply' + job.id)" data-dismiss="modal">Close</button>
                            <a type="button" class="btn btn-primary" :href="'https://crossedlink.com/me/'">My Profile</a>
                            </div>
                        </div>
                        </div>
                  </div>  
    ...
    <div v-else>
        <span class="m-5 empty-to-show box w-100">
                No Jobs To Show
                <a href= "https://crossedlink.com/post/job" v-if="role==2 || role == 1">Post Job</a>
        </span> 
    </div>
</div>
</template>
                  ...
                  $.ajax({
                      ...
                      success: (response)=>{
                        console.log('done');
                          ...
                          $('.toast').find(".message").text('Job Applied !');
                          $(id).find("button").removeClass('btn-primary').addClass('btn-success').text('Unapply');
                          setTimeout(function () {
                ...
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                  ...
                  success: (response)=>{
                    console.log($(id).serialize());
                      ...
                      $(id).find("button").removeClass('btn-success').addClass('btn-primary').text('Apply now');
                      $('.toast').fadeIn().removeClass('hide').addClass('show');
                      $('.toast').find(".message").text('Unapplied job !');
                      setTimeout(function () {
                        $('.toast').fadeOut().removeClass('show').addClass('hide');
                    }, 5000);
                  }
