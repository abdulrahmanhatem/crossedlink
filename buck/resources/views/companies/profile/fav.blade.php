@include('inc.home.head', ['title' => trans('main.Saved List')])
    <section class="section p-150 bread-crumbs">
        <div class="container">
            <a href="{{ url('/') }}" class="text-muted">{{trans('main.Home')}}</a> / <a href="{{ url('unlock') }}" class="text-primary">{{trans('main.Saved List')}}</a>
        </div>
    </section>
                ...
                <div class="col-lg-3 col-md-12  my-4">
                    <div class="left-sidebar ">
                        <ul class="nav nav-pills nav nav-pills bg-white rounded" id="pills-tab" role="tablist">
                            ...
                            <li class="nav-item d-block col-12">
                                <a class="nav-link active" id="unblock-tab"  href="{{ url('favorite') }}" >{{trans('main.Saved List')}}(<span class="count">{{ count(Helper::favList(auth()->user()->id)) }}</span>)</a>
                            </li>
                            <li class="nav-item d-block col-12 ">
                        ...
                        <div class="tab-pane fade show active general">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            {!! Form::open(['action'=> ['ProfileController@update', $user->id], 'method'=>'POST', 'enctype' =>'multipart/form-data', 'class' => 'w-100'])!!}
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    {!! Form::submit(trans('main.Delete'), ['class' => "btn btn-primary btn-sm", 'name' => 'delete-fav']) !!}
                                                </div>
                                            {!! Form::text('worker_id', $worker->id, ['hidden']) !!}
                                            {!! Form::hidden('_method', 'PUT') !!}
                                            {!! Form::close() !!}
                            ...
                            @endforeach
                                @else
                                        ...
                                        <h6 class="my-5 text-center w-100 ">{{trans('main.Save Some')}} <a href= "{{ url('search/workers') }}"> {{trans('main.Candidate Profiles!')}}</a></h6>
                                @endif
                            </div>
                            <div class="row">
              ...
              $.ajax({
                  ...
                  success: (response)=>{
                     $('.count').html(parseInt($('.count').html()) - 1);
                      ...
                      $(this).closest('.experience-view').fadeOut(1000);
