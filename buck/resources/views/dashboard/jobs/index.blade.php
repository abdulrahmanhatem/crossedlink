...
<div class="page  has-sidebar-left height-full job-list company-list">
                ...
                <ul class="nav nav-material nav-material-white responsive-tab" id="v-pills-tab" role="tablist">
                    <li>
                        <a class="nav-link active"  href="{{url('dashboard/jobs')}}"
                           ><i class="icon icon-home2"></i>All Jobs</a>
                    </li>
                    ...
                    <li class="float-right search-li">
                        {!! Form::open(['action'=>'JobController@search', 'method'=>'GET', 'enctype' =>'multipart/form-data', ' novalidate', 'class' => 'needs-validation d-block'])!!}
                                    ...
                                    <form>
                                        <table class="table table-striped table-hover r-0">
                                            ...
                                            <tbody>
                                                    ...
                                                    @foreach ($jobs as $job)
                                                        @if(Helper::checkUser($job->employer_id))
                                                        
                                                        <tr>
                                                            <td>{{$job->title}}</td>
                                                            <td>
                                                                @if($job->employer->role == 2)
                                                                <a href="{{url('dashboard/companies/'.$job->employer->id)}}">
                                                                @elseif($job->employer->role == 1)
                                                                    <a href="{{url('dashboard/personal/'.$job->employer->id)}}"> 
                                                                @endif
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <small> {{ Helper::getCategoryName($job->category_id) }}</small>
                                                                    </div>
                                                                </a>
                                                            </td>
